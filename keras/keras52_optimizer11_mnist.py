...
# 검증셋을 원본에서 먼저 떼어냄
# validation_split은 섞기 전 마지막 20%를 가져가는데, 증폭본을 뒤에 붙이면 검증셋이 전부 증폭본이 됨
# 테스트가 원본이므로 검증도 원본이어야 하고, 증폭은 훈련셋에만 적용
...
# reshape 이유 : OneHotEncoder는 2차원 입력만 받기 때문
...
mcp = ModelCheckpoint(
    ...
    mode='min',
    save_best_only=True,
    filepath=path + 'keras52_11_mnist.keras'
)
...
print('loss : ', loss[0])                                   # loss : 0.032467007637023926 > 0.03474234417080879
print('acc : ', loss[1])                                    # acc : 0.9941999912261963 > 0.9905999898910522
...
print('accuarcy_score : ', acc_score)                       # accuarcy_score: 0.9942 > 0.9906
print('time : ', round(end_time - start_time, 2), 'sec')    # time : 652.79 sec > 1680.93 sec